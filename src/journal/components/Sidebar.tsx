...
export const JournalSidebar = connect()(
  class extends React.Component<JournalState> {
      ...
      return (
        <div className="menu">
          {this.props.sidebar.TagLinks.map((l, i) => (
            ...
            >
              <a href={`#tag/${l.Name}`}>
                #{l.Name} <span className="counter">({l.Count})</span>
              </a>
            </div>
          ))}
        </div>
    ...
    renderAlphabetically() {
      if (this.props.sidebar.ChronoLinks === null) {
        return (
          <p>No alphabetical links yet; have you titled any journal entries?</p>
        );
      }
      ...
      const convertTable = (table: any) => {
        const keys: string[] = Object.keys(table).filter(
          k => k !== "title" && k !== "link"
        );
        const obj: any = {};
        ...
        if (keys.length > 0)
          obj.children = keys.map(k => convertTable(table[k]));
        return obj;
      };
    ...
    renderChronologically() {
      if (
        !this.props.sidebar.ChronoLinks ||
        this.props.sidebar.ChronoLinks.length === 0
      ) {
        return (
          <p>
            No chronological links yet. Have you created any journal entries?
          </p>
        );
      }
          ...
          <div>
            ...
            <Tabs defaultIndex={defaultTab}>
              <TabList>
                ...
                <Tab>
                  <OcticonSpan icon={OcticonTextSize} />
                  Title
                </Tab>
                <Tab>
                  <OcticonSpan icon={OcticonTag} />
                  Tag
                </Tab>
              </TabList>

              <TabPanel>{this.renderChronologically()}</TabPanel>
              <TabPanel forceRender={true}>
                {this.renderAlphabetically()}
              </TabPanel>
              <TabPanel>{this.renderTags()}</TabPanel>
            </Tabs>
          </div>
        );
      }
