...
import { generateGutterTimestamps, TIME_DIFF_THRESHOLD_MINUTES } from './gutters'
...
describe('generateGutterTimestamps', () => {
  it('shows timestamp for first line', () => {
    const lines: ZLine[] = [
      {
        ...lineMake(0),
        updatedAt: '2024-08-18T17:30:00.000Z',
      },
    ]
  ...
  it('returns null for subsequent lines within time threshold', () => {
    ...
    const lines: ZLine[] = [
      ...
      {
        ...lineMake(0),
        updatedAt: new Date(baseTime.getTime() + 10 * 60 * 1000).toISOString(), // 10 minutes later
      },
    ]
  ...
  it('shows time when difference exceeds threshold', () => {
    ...
    const lines: ZLine[] = [
      ...
      {
        ...lineMake(0),
        updatedAt: new Date(baseTime.getTime() + (TIME_DIFF_THRESHOLD_MINUTES + 5) * 60 * 1000).toISOString(),
      },
    ]
  ...
  it('shows full date when date differs', () => {
    const lines: ZLine[] = [
      ...
      {
        ...lineMake(0),
        updatedAt: '2024-08-19T09:15:00.000Z', // Next day
      },
    ]
  ...
  it('handles multiple lines with mixed scenarios', () => {
    ...
    const lines: ZLine[] = [
      ...
      {
        ...lineMake(0),
        updatedAt: new Date(baseTime.getTime() + 5 * 60 * 1000).toISOString(), // 5 minutes later - should be null
      },
      {
        ...lineMake(0),
        updatedAt: new Date(baseTime.getTime() + 25 * 60 * 1000).toISOString(), // 25 minutes later - should show time
      },
      {
        ...lineMake(0),
        updatedAt: '2024-08-19T10:00:00.000Z', // Next day - should show full date
      },
    ]
  ...
  it('handles exact threshold boundary', () => {
    ...
    const lines: ZLine[] = [
      ...
      {
        ...lineMake(0),
        updatedAt: new Date(baseTime.getTime() + TIME_DIFF_THRESHOLD_MINUTES * 60 * 1000).toISOString(), // Exactly threshold
      },
    ]
