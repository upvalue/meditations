...
var _mediumEditorTables = __webpack_require__(3);

var _mediumEditorTables2 = _interopRequireDefault(_mediumEditorTables);
...
var Common = {
  ...
  request: function request(data) {
    var ret = _jquery2.default.extend({ type: "POST", contentType: "application/json; charset=UTF-8" }, data);
    ret.data = JSON.stringify(ret.data);
    return _jquery2.default.ajax(ret);
  },
  ...
  initialize: function initialize() {
    ...
    var show_back = _mediumEditorTables2.default.prototype.show;
    _mediumEditorTables2.default.prototype.show = function () {
      (0, _jquery2.default)(".fa").removeClass("fa").addClass("octicon");
      var fareplace = function fareplace(bef, aft) {
        return (0, _jquery2.default)('.fa-' + bef).removeClass('fa-' + bef).addClass('octicon-' + aft);
      };
   ...
   */
  make_editor: function make_editor(selector, focus, blur) {
    var args = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};

    var editor = Common.editor = new _mediumEditor2.default(selector, _jquery2.default.extend({
      // Default arguments
      autoLink: true,
      placeholder: false,
      extensions: { table: new _mediumEditorTables2.default() },
      toolbar: { buttons: ['bold', 'italic', 'underline', 'anchor', 'image', 'quote', 'orderedlist', 'unorderedlist', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table'] }
    }, args));

    // Register callbacks
    editor.subscribe("focus", function () {
      (0, _jquery2.default)("#saved-button").removeClass("octicon-check");
      (0, _jquery2.default)("#saved-button").addClass("octicon-circle-slash");
      (0, _jquery2.default)(window).on("beforeunload", focus);
    });

    editor.subscribe("blur", function () {
      (0, _jquery2.default)("#saved-button").addClass("octicon-check");
      (0, _jquery2.default)("#saved-button").removeClass("octicon-circle-slash");
      blur();
      (0, _jquery2.default)(window).off("unload");
    });
   ...
   * @returns {WebSocket}
   */
  make_socket: function make_socket(location, onmessage) {
    var protocol = window.location.protocol == 'https:' ? 'wss' : 'ws';
    var url = protocol + '://' + window.location.hostname + ':' + window.location.port + '/' + location;

    var socket = new WebSocket(url);
    socket.onopen = function (m) {
      return console.log('Common.make_socket: Connected to ' + url + ' websocket');
    };
    socket.onmessage = function (m) {
      return onmessage(JSON.parse(m.data));
    };
    return socket;
  },
   ...
   */
  routerInitialize: function routerInitialize(base, first, routes) {
    window.route = _riotRoute2.default; // TODO remove after journal is ported
    ...
    (0, _riotRoute2.default)(function () {
      ...
      var action = [].shift.apply(arguments);
    ...
    // Initialize router
    _riotRoute2.default.base(base);
    _riotRoute2.default.start(true);

    if (window.location.hash.length <= 2) {
      if (first) {
        (0, _riotRoute2.default)(first);
      }
    }
  },
   ...
   */
  register_events: function register_events(obj) {
    var events = [];
    ...
    try {
      for (var _iterator = Object.getOwnPropertyNames(obj.__proto__)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var key = _step.value;
...
// medium editor
window.MediumEditor = __webpack_require__(4);
window.MediumEditorTable = __webpack_require__(3);

// riot & etc
window.RiotControl = __webpack_require__(14);
window.riot = __webpack_require__(13);

window.Common = __webpack_require__(1).default;
...
var Journal = __webpack_require__(21).default;
 ...
 */
var process_entry = function process_entry(entry) {
...
var Journal = {
   ...
   * @class
   */
  EntryStore: function (_Common$Store) {
    _inherits(EntryStore, _Common$Store);

    function EntryStore() {
      _classCallCheck(this, EntryStore);

      var _this = _possibleConstructorReturn(this, (EntryStore.__proto__ || Object.getPrototypeOf(EntryStore)).call(this));

      _common2.default.register_events(_this);
      return _this;
    }
    ...
    _createClass(EntryStore, [{
      key: 'on_delete_entry',
      value: function on_delete_entry(id) {
        $.post({
      ...
      key: 'on_journal_update',
      value: function on_journal_update(entry) {
        _common2.default.request({
          url: "/journal/update",
          data: entry
        });
      }
      ...
      key: 'on_name_entry',
      value: function on_name_entry(id, name) {
        $.post({
      ...
      key: 'on_browse_tag',
      value: function on_browse_tag(name) {
        (0, _riotRoute2.default)('tag/' + name);
      }
      ...
      key: 'on_remove_tag',
      value: function on_remove_tag(entry_id, tag) {
      ...
      key: 'on_add_tag',
      value: function on_add_tag(entry_id, tag) {
        $.post({ url: '/journal/add-tag/' + entry_id + '/' + tag });
      }
    }]);

    return EntryStore;
  }(_common2.default.Store),
  ...
  name: function name(_name) {
    ...
    console.log(_name);
    $.get('/journal/entries/name/' + _name, function (entry) {
      entry = process_entry(entry);
      document.title = _name + ' /journal';
  ...
  tag: function tag(name) {
    console.log('Tag');
    $.get('/journal/entries/tag/' + name, function (entries) {
   ...
   */
  view: function view(datestr, entry_scroll_id) {
    var date = moment(datestr, 'YYYY-MM');

    document.title = date.format('MMMM YYYY') + ' / journal';

    $("#habits-link").attr("href", '/habits#view/' + date.format('YYYY-MM') + '/0');

    $.get('/journal/entries/date?date=' + datestr, function (entries) {
      ...
      riot.mount('entries', {
        ...
        thunk: function thunk() {
          if (entry_scroll_id) {
            ...
            var offset = $('#entry-' + entry_scroll_id).offset().top;
  ...
  main: function main() {
    _common2.default.initialize();
    var entry_store = new Journal.EntryStore();

    RiotControl.addStore(entry_store);

    $("#journal-new-entry-date").datepicker({
      onSelect: function onSelect(datestr) {
        var date = moment(datestr, "MM/DD/YYYY").format("YYYY-MM-DD");
        $.post('/journal/new?date=' + date, function () {
          view(moment(date, 'YYYY-MM-DD').format('YYYY-MM'));
        });
      }
    });

    _common2.default.routerInitialize("/journal#", 'view/' + moment().format('YYYY-MM'), {
      view: Journal.view,
      tag: Journal.tag,
      name: Journal.name
    });

    var socket = _common2.default.make_socket("journal/sync", function (entry) {
...
riot.tag2('entry', '<h5 class="entry-title"> <span ref="title"></span> <span class="journal-controls float-right"> <span class="float-right"> <button if="{!NoContext}" class="journal-control btn btn-link btn-sm" title="Context"> <a href="#view/{moment(this.Date, \'YYYY-MM-DD\').format(\'YYYY-MM\')}"><span class="octicon octicon-link"></span></a> </button> <button class="journal-control btn btn-link btn-sm octicon octicon-text-size" title="Edit name" onclick="{name_entry}"></button> <button class="journal-control btn btn-link btn-sm octicon octicon-tag" title="Add tag" onclick="{add_tag}"></button> <button class="journal-control btn btn-link btn-sm octicon octicon-x" title="Delete" onclick="{delete_entry}"></button> </span> <br> <div class="journal-timestamp float-right"> <a href="#view/{Context}/{ID}" title="{moment(CreatedAt, \'YYYY-MM-DD\\Thh:mm\').format(\'M-D-YY hh:mm A\')}"><em>{moment(CreatedAt, \'YYYY-MM-DD\\Thh:mm\').format(!NoContext ? \'M-D-YY hh:mm A\' : \'hh:mm A\')}</em></a> </div> <div class="journal-tags float-right"> <span class="journal-tag float-right" each="{Tags}"> <a href="#tag/{Name}">#{Name}</a> <button class="btn btn-xs octicon octicon-x" title="Remove tag" onclick="{remove_tag}" data-name="{Name}"></button> </span> </div> </span> </h5> <div id="{⁗entry-body-⁗+ID}" ref="body" class="entry-body"> </div> <hr>', '', 'id="{⁗entry-⁗+ID}"', function(opts) {
...
riot.tag2('title-nav-sub', '', '', '', function(opts) {
});

riot.tag2('title-nav', '<ul id="alphabetical-top-list" class="list-group navigation-list"> <li each="{opts.links}"> <span if="{typeof sub == ⁗undefined⁗ && name}">{name}</span> <span if="{typeof sub != ⁗undefined⁗}"> <a href="#">{name}</a> <ul> </ul> </span> </li> </ul>', '', '', function(opts) {
...
/***/ })
/******/ ]);
//# sourceMappingURL=bundle-journal.js.map