...
+ linktree.tsx contains a component that is a tree of links. It's used to navigate journal named
  entries.
+ journal-sidebar.tsx contains the Journal sidebar component.
...
the end of habitsweb.go and journal.go, and the end of habits.tsx and journal.tsx, which is where
the routing logic and initialization for the backend and frontend live.
